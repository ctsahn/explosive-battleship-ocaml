...
let handle_user_turn (user_move:string) request = 
  ...
  Dream.log "jello %s" (Board.board_to_string cpu_board);
  Dream.html (Template.game_board  ~user_board_status:(Board.board_to_string user_board) ~cpu_board_status:(Board.board_to_string cpu_board) ~turn:"cpu" request ) (* Load CPU turn after processing user move *)
...
let () =
  ...
  @@ Dream.router [
    ...
    Dream.get  "/"
      (fun request ->
        Dream.html  (Template.game_board  ~user_board_status:(Board.board_to_string user_board) ~cpu_board_status:(Board.board_to_string cpu_board) ~turn: "user" request )); (* Start out with user turn with a blank message/board status*)
